...
#        embedding_model = gensim.models.keyedvectors.load_word2vec_format(model_name, binary=False, allow_pickle=True)
...
def write_output(write_path, y_pred, perm):
    ...
    with open(os.path.join(write_path, 'out.txt'), 'w') as f:
        for vals in y_pred:
            f.write(','.join(map(lambda val:str(float(val)), vals)) + '\n')
