...
from model_lib.base import BaseFeatureExtractor
...
class FeatureExtractor(BaseFeatureExtractor):
    def __init__(
        ...
        self.lm_model = transformers.RobertaModel.from_pretrained(consts.LM_NAME).eval().to(device)

    def _get_model_outputs(self, marked_sents):
        input_idmasks_spans_batches = self._batchify(marked_sents, device=self.device)
        with torch.no_grad():
            for input_ids_batch, input_masks_batch, _ in tqdm(input_idmasks_spans_batches, ncols=100, desc='inference'):
                model_output = self.lm_model(
                    input_ids_batch,
