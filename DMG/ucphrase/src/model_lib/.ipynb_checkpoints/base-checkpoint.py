...
class BaseModel(nn.Module):
    ...
    @classmethod
    def _from_config(cls, device, config_dict: dict):
        raise NotImplementedError

    @classmethod
    def from_config(cls, device, config_or_path_or_dir_or_dict):
        config = None

        if isinstance(config_or_path_or_dir_or_dict, dict):
            config = config_or_path_or_dir_or_dict
        else:
            assert isinstance(config_or_path_or_dir_or_dict, str) or isinstance(config_or_path_or_dir_or_dict, Path) 
            path_or_dir = Path(config_or_path_or_dir_or_dict)
        ...
        return cls._from_config(device, config).to(device)
    ...
    def predict(self, path_predict_docs, dir_output, device, batch_size, use_cache):
        raise NotImplementedError
    ...
    def _par_decode_doc(predicted_doc, threshold):
        ...
        for predicted_sent in predicted_doc['sents']:
            ...
            # predicted_spans = sorted(predicted_spans, key=lambda s: (s[2], s[1] - s[0]), reverse=True)
            idxs_taken = set()
            spans = []
    ...
    @classmethod
    def load_ckpt(cls, device, path_ckpt):
        print(type(path_ckpt), path_ckpt)
        ckpt = torch.load(path_ckpt, map_location='cpu')
        
        path_tups = path_ckpt.split('/')
        path_model_config = '/'.join(path_tups[:-1]) + '/model_config.json'
        print(path_model_config)
        model = cls.from_config(device, path_model_config)
...
class BaseFeatureExtractor:
    ...
    @ staticmethod
    def _batchify(marked_sents, device, is_train=False):
        ...
        while pointer < total_num:
            ...
            input_masks_batch = torch.tensor(input_masks_batch, device=device)
            if is_train:
