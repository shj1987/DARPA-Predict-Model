...
from data_lib.annotator_base import BaseAnnotator
...
STPWD_SET = set(utils.TextFile.readlines('./ucphrase/stopwords.txt'))
...
class CoreAnnotator(BaseAnnotator):
    ...
    def _par_mine_doc_phrases(doc_tuple):
        ...
        for p in phrases:
            ...
            for cp in longer_phrases:
                if p in cp:
                    has_longer_pattern = True
                    break
            if not has_longer_pattern:
                if len(p.split()) <= MAXGRAMS:
                    cleaned_phrases.add(p)
                longer_phrases.add(p)
