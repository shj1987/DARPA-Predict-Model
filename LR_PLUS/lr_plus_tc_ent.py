...
def train(main_data, narratives, leidos, zipf):
    p = 0
    narrative_coef = {}
    ...
    for na in narratives:
        ...
        for k in range(p, len(current_main)):
            ...
            x = list(np.multiply(current_x[k - p:k + 1], current_zipf[k - p:k + 1]))
            data_x.append(x)
...
def main(argv):
    parser = argparse.ArgumentParser(description='LR PLUS TCENT model.')
    parser.add_argument('-m', '--main_file', required=True, type=str)
    parser.add_argument('-g', '--tc_file', required=True, type=str)
    parser.add_argument('-z', '--zipf_file', required=True, type=str)
    ...
    narrative_coef = train(main_data, narratives, leidos, zipf)
    ...
    for na in narratives:
        ...
        current_zipf = zipf[na][train_num - p: train_num + test_num].to_list()
        for d in range(test_num):
            leidos_predict = list(np.multiply(current_x[d : d + p + 1], current_zipf[d : d + p + 1]))
