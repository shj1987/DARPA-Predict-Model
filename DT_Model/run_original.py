...
def main():
    ...
    for key in nodelist:
        ...
        for target in targets:
            name = key.replace('/','#')
            fname = name + "_original_803_train.csv"
            data_csv_data_filename = '2021_' + os.path.join(target,fname)
            ...
            sdate = 1596499200000 #2020-7-14 # 3-22 1552608000000 #3-15 1552003200000 # 3-8 #1551398400000 #3-1 #1550188800000# 2-14 #1549584000000
    ...
    #print("rmse:",  error[0])
    #print("ape:", error[1])
    with open('youtube_UIUC_ML_REGRESS_CORR_STR_MATCH_10.json', 'w') as outfile:
        json.dump(yt10, outfile)
    with open('twitter_UIUC_ML_REGRESS_CORR_STR_MATCH_10.json', 'w') as outfile:
        json.dump(tt10, outfile)
    with open('youtube_UIUC_ML_REGRESS_CORR_STR_MATCH.json', 'w') as outfile:
        json.dump(yt5, outfile)
    with open('twitter_UIUC_ML_REGRESS_CORR_STR_MATCH.json', 'w') as outfile:
        json.dump(tt5, outfile)
    with open('youtube_UIUC_linear_avg_dryrun.json', 'w') as outfile:
        json.dump(yt, outfile)
    with open('twitter_UIUC_linear_avg_dryrun.json', 'w') as outfile:
...
def plot_model_tree_fit(model, X, y, name,  target, error):
        ...
        for i in range(bag):
            ...
            data_csv_data_filename = '2021_' + os.path.join(target, name+"_original_803_test.csv")
