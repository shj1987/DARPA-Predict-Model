...
def json_to_csv(data_file, date, platform, time_series):
    ...
    df_event.to_csv(f'./{data_file}_csv/' + platform + f'_{t}_event.csv', index_label='InfoID')
    df_user.to_csv(f'./{data_file}_csv/' + platform + f'_{t}_user.csv', index_label='InfoID')
    df_newuser.to_csv(f'./{data_file}_csv/' + platform + f'_{t}_newuser.csv', index_label='InfoID')
...
def gdelt_to_csv(data_file, gdelt_file):
    ...
    df.to_csv(f'./{data_file}_csv/gdelt.csv', index_label='InfoID')
...
def corr_to_csv(data_file, date, corr_file, platform):
    ...
    df_twitter.to_csv(f'./{data_file}_csv/corr_{t}_{platform}.csv')
...
def generate_training_data(platform, section, corr, nodelist, training_length, prediction_length):
    ...
    # read correlation
    df_corr = pd.read_csv(f'./{data_file}_csv/corr_{t}_' + platform + '.csv', header=0, index_col=0)
    ...
    active_event = sorted(set(active_event_1).intersection(set(active_event_2)))
