...
python timeseries_gdelt.py \
    ...
    -g D \
    -s "$START_DATE" -e "$END_DATE" \
    -o "$DATA_ROOT/workdir/cp6_gdelt_timeseries.json"
python timeseries_acled.py \
    -i "$DATA_ROOT/Exogenous/ACLED/cp6.ea.acled.v1.csv.gz" \
    -g D \
    -s "$START_DATE" -e "$END_DATE" \
    -o "$DATA_ROOT/workdir/cp6_acled_timeseries.json"
...
python timeseries_platform.py \
    ...
    -p "twitter" \
    -s "$START_DATE" -e "$END_DATE" \
    -o "$DATA_ROOT/workdir/cp6_twitter_timeseries.json" \
    --statout "$DATA_ROOT/workdir/cp6_twitter_prob.json"
python timeseries_platform.py \
    -i "$DATA_ROOT/workdir/cp6.ea.gte.twitter-youtube.2020-11-30_2020-12-20.json_youtube.json" \
       "$DATA_ROOT/workdir/cp6.ea.gte.twitter-youtube.2020-12-21_2021-01-10.json_youtube.json" \
    -n "$DATA_ROOT/cp6_twitter_nodelist.txt" \
    -p "youtube" \
    -s "$START_DATE" -e "$END_DATE" \
    -o "$DATA_ROOT/workdir/cp6_youtube_timeseries.json" \
    --statout "$DATA_ROOT/workdir/cp6_youtube_prob.json"
